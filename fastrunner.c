...
int main(int argc, char **argv) {
    ...
    int64_t V[2][5] = {{1,2,3,4,5},{1,3,4,5,9}};
    for (int t = 0; t < 2; t++) {
        long nt = 0, bad = 0; int64_t maxc = 0;
        #pragma omp parallel for schedule(dynamic,16) reduction(+:nt,bad)
        for (int64_t c = 1; c <= CMAX; c++) {
            int skip = 0; for (int i = 0; i < 5; i++) if (V[t][i] == c) skip = 1;
            if (skip) continue;
            int64_t v[6]; for (int i = 0; i < 5; i++) v[i] = V[t][i]; v[5] = c;
            if (!near_tight(v, 6, 6)) continue;
            int64_t p, q, Q; ml_exact(v, 6, &p, &q, &Q);
            int64_t k = q - 6 * p;
            nt++;
            if (k != 1 || p * 6 != c || q != c + 1) { bad++;
                #pragma omp critical
                printf("  k != 1: c=%lld ML=%lld/%lld k=%lld\n", (long long)c,(long long)p,(long long)q,(long long)k); }
            #pragma omp critical
            { if (c > maxc) maxc = c; }
        }
        printf("V=(%lld,%lld,%lld,%lld,%lld): c <= %lld: near-tight sextuples %ld, with k != 1: %ld, largest near-tight c %lld\n",
            (long long)V[t][0],(long long)V[t][1],(long long)V[t][2],(long long)V[t][3],(long long)V[t][4],
